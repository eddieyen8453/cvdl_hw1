...
class Question1:
    ...
    def findCorner(self, dirName):
        ...
        filePaths = glob.glob(os.path.join(dirName, "*.bmp"))  
        filePaths.sort(key = lambda x: int(x[len(dirName)+1:-4])) 
        # print(filePaths)
    ...
    def findIntrinsic(self, dirName):
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)  # termination criteria
        global dist, mtx # global call for findDistortion(1.4)
    ...
    def findExtrinsic(self, dirName, n):
        ...
        objPoint[:,:2] = np.mgrid[0:self.w, 0:self.h].T.reshape(-1, 2)
        print('path : ',dirName + '/' + str(n) + '.bmp')
        # print('path : ',dirName + '/' + n + '.bmp')
        imgGray = cv2.imread(dirName + '/' + str(n) + '.bmp', 0) # read image in grayscale
