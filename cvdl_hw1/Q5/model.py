...
def train():
    ...
    model = torchvision.models.vgg19_bn(num_classes = 10 ) # using vgg19_bn
    model.to(device)
    ...
    plt.title('Accuracy')
    plt.ylabel('%')
    plt.plot(train_accuracy, color='blue', label="train_acc")
    plt.plot(val_accuracy, color='orange', label="val_acc")
    plt.legend()
    plt.savefig('Q5/accuracy.png')
    # plt.show()
    ...
    plt.legend()
    plt.savefig('Q5/loss.png')
    # plt.show()

    summary(model, (3, 224, 224))   # show model structure

    torch.save(model, 'Q5/model_vgg19.pth')    # save the model
...
def predict_image(imgPath):
    if imgPath == None:
        ...
        # print(eval ,float(conf), class_names[classes.item()])
        return eval, float(conf), class_names[classes.item()]
...
if __name__ == '__main__':
    train()
    # imgPath = "/home/fanal/disk2/eddie/transtab/Q5/data/Q5_4/airplane.png"
